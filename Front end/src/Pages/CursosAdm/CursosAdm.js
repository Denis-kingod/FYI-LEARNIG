...
export default class CursosAdm extends Component {
  ...
  buscaCategoriums = () => {
    axios("https://fyilearningsolutions.azurewebsites.net/api/CategoriumsControllers", {
      headers: {
  ...
  buscarCursos = () => {
    axios("https://fyilearningsolutions.azurewebsites.net/api/CursosControllers")
      .then((resposta) => {
        if (resposta.status === 200) {
  ...
  cadastrarCursos = (event) => {
    ...
    axios
      .post("https://fyilearningsolutions.azurewebsites.net/api/CursosControllers", curso, {
        haders: {
          autorization: "Bearer" + localStorage.getItem("usuario-login"),
        },
      })
